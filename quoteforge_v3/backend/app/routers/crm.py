...
async def quick_generate(
    ...
    # Upload the PDF to Salesforce as a ContentVersion + create a
    # QuoteForge_Document__c record + log activity. create_quoteforge_document
    # is the full PandaDoc-style flow (file in SF Files related list,
    # custom-object record with pricing/metadata, activity timeline entry).
    # All wrapped in try/except so a SF-side failure doesn't 500 the API
    # response — the proposal is already generated and persisted server-side.
    try:
        from app.models.document_log import DocumentLog
        doc_row = (await db.execute(
            select(DocumentLog).where(DocumentLog.doc_id == result["doc_id"])
        )).scalar_one_or_none()
        if doc_row and doc_row.file_path:
            doc_metadata = {
                "doc_id": result["doc_id"],
                "title": f"{result['deal_name']} — {result['type']}",
                "description": f"Generated by QuoteForge Quick Trigger ({identifier})",
                "type": result.get("type", "Proposal"),
            ...
            await sf.create_quoteforge_document(
                opportunity_id=opp_id,
                file_path=doc_row.file_path,
                doc_metadata=doc_metadata,
            )
        else:
            # Fallback: at minimum drop an activity timeline entry.
            await sf.log_activity(
                opportunity_id=opp_id,
                subject="QuoteForge: Proposal Generated (Quick Trigger)",
                description=f"Triggered via deal identifier: {identifier}\nDocument: {result['doc_id']}",
            )
    except Exception:
        logger.exception("quick_generate: SF upload failed (proposal still returned to caller)")
